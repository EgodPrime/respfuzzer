...
from tracefuzz.agentic_function_resolver import solve, Attempter, QueitExecutor, Reasoner, Judger
from tracefuzz.models import ExecutionResultType, Function, Argument
import tracefuzz.agentic_function_resolver as afr


class MockFunction(Function):
    def __init__(self):
        super().__init__(id=1, func_name="ast.literal_eval", source="mock", args=[Argument(arg_name="s", type="str", pos_type="POSITIONAL_OR_KEYWORD")])
...
def test_solve_retries_and_returns_code(monkeypatch):
    ...
    exec_ = mock.Mock(side_effect=[
        {"result_type": ExecutionResultType.OK, "ret_code": 0, "stdout": "ok", "stderr": ""},
    ])
    ...
    judge = mock.Mock(side_effect=[{"valid": False, "reason": "no call"}, {"valid": True, "reason": "ok"}])
...
def test_solve_returns_none_when_budget_exhausted(monkeypatch):
    ...
    exec_ = mock.Mock(return_value={"result_type": ExecutionResultType.ABNORMAL, "ret_code": 1, "stdout": "", "stderr": "err"})
    ...
    monkeypatch.setattr(Judger, "judge", mock.Mock(return_value={"valid": False, "reason": "no call"}))
...
def test_solve_handles_attempter_errors_then_succeeds(monkeypatch):
    ...
    exec_ = mock.Mock(return_value={"result_type": ExecutionResultType.OK, "ret_code": 0, "stdout": "ok", "stderr": ""})
    ...
    assert any(h.get('role','').startswith('attempter') or h.get('role','')=='attempter_error' for h in captured['history'])
...
def test_solve_handles_judger_exception_then_succeeds(monkeypatch):
    ...
    judge = mock.Mock(side_effect=[Exception("judge fail"), {"valid": True, "reason": "ok"}])
    exec_ = mock.Mock(return_value={"result_type": ExecutionResultType.OK, "ret_code": 0, "stdout": "ok", "stderr": ""})
...
def test_solve_retries_after_abnormal_execution(monkeypatch):
    ...
    exec_ = mock.Mock(side_effect=[
        {"result_type": ExecutionResultType.ABNORMAL, "ret_code": 1, "stdout": "", "stderr": "ValueError"},
        {"result_type": ExecutionResultType.OK, "ret_code": 0, "stdout": "ok", "stderr": ""},
    ])
